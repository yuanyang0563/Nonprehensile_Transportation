...
class manipulators {
  ...
  public:
	...
	void get_pose_jacobian () {
		...
		getJoints = left.getJoint && right.getJoint;
	}
	
	void control_forward () {
		left.control_forward();
		right.control_forward();
	}
	
	void plan_paths (int num) {
		left.plan_path(num);
		right.plan_path(num);
		for (int i=0; i<=num; ++i) {
			right.xs[i] = left.xs[i]+left.Rs[i]*left.Rs[0].transpose()*(right.xs[0]-left.xs[0]);
			right.Rs[i] = left.Rs[i];
		}
		if (!getPaths)
			getPaths = !getPaths;
	}
	...
	void set_waypoints () {
		waypoint += 1;
		left.xt = left.xs[waypoint];
		left.Rt = left.Rs[waypoint];
		right.xt = right.xs[waypoint];
		right.Rt = right.Rs[waypoint];
	}
